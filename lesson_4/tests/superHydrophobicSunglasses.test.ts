...
test.describe('PDPInteraction events', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    const sunglassesButton = await page.waitForSelector(
      '//nav//a[contains(., "Sunglasses")]'
    );
    await Promise.all([
      sunglassesButton.click(),
    ...
    await page.waitForTimeout(5000);
    const chooseLenses = await page.waitForSelector(
    ...
    let continueButton = await page.waitForSelector(
  ...
  test('should fire after adding coating and removing it', async ({
    ...
    let backToPrev = await page.waitForSelector(
      '//button[text() = "Back"]'
    );
    await backToPrev.click();

    let hydrophobicButton = await page.waitForSelector(
    ...
    backToPrev = await page.waitForSelector(
      '//button[text() = "Back"]'
    );
    await backToPrev.click();

    hydrophobicButton = await page.waitForSelector(
      'input[value="Super Hydrophobic"]'
    );
    await hydrophobicButton.click();
